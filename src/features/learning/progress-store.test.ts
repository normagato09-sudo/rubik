...
const F2L_IDS = F2L_CASES.map((f2lCase) => caseItemId("f2l", f2lCase.id));
...
describe("learning progress", () => {
  it("starts at 0/24 and counts each learned case once", () => {
    ...
    store.getState().toggleLearned(caseItemId("f2l", "01"));
    expect(countLearned(store.getState().learned, F2L_IDS)).toBe(1);

    store.getState().toggleLearned(caseItemId("f2l", "02"));
    expect(countLearned(store.getState().learned, F2L_IDS)).toBe(2);

    store.getState().toggleLearned(caseItemId("f2l", "01"));
    expect(countLearned(store.getState().learned, F2L_IDS)).toBe(1);
    expect(store.getState().isLearned(caseItemId("f2l", "02"))).toBe(true);
  });
  ...
  it("persists across a reload under its own key", async () => {
    ...
    first.getState().toggleLearned(caseItemId("f2l", "07"));
    expect([...storage.data.keys()]).toEqual([LEARNING_PROGRESS_STORAGE_KEY]);

    const reloaded = createLearningProgressStore(() => storage);
    expect(reloaded.getState().isLearned(caseItemId("f2l", "07"))).toBe(false);
    await reloaded.persist.rehydrate();
    expect(reloaded.getState().isLearned(caseItemId("f2l", "07"))).toBe(true);
